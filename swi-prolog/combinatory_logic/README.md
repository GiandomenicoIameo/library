...
false.
```
Come conseguenza dei due precedenti predicati, è anche possibile utilizzare lo stesso predicato con il solo scopo di verificare che un insieme `A` sia sottoinsieme di un certo insieme `B` ( realazione di inclusione ). E' sufficiente scrivere la query nel seguente modo :
